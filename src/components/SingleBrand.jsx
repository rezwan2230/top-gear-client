...
const SingleBrand = () => {
    ...
    return (
        <div>
            ...
            {
                found.length > 0 ? <section className="px-4 md:px-6 py-10 md:pt-24 mx-auto lg:container">
                    <div className="grid grid-cols-1 gap-4 lg:gap-12 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 lg:mt-10">
                        {
                            products?.map(product => <SingleProduct key={product._id} product={product}></SingleProduct>)
                        }
                ...
                : <div className="flex justify-center items-center my-52">
                    <p className="text-5xl font-semibold">The collection of this brand has not been added yet</p>
                </div>
            }
        </div>
    );
